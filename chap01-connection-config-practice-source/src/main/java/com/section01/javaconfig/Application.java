    ...
    public static void main(String[] args) {
        ...
        *  */
        Environment environment = new Environment(
                "dev",                          //환경 정보 이름
                new JdbcTransactionFactory(),       // 트랜잭션 매니저 종류 결정 (JDBC or MANAGER)
                new PooledDataSource(Driver, URL, USER, PASSWORD) //ConnecgionPool 사용 유무 (Pooled or UnPooled)
        );
        ...
        *          SqlSessionFactory 인터페이스 타입의 객체를 반화낳는 메소드*/
        SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(configuration);

        /* openSession(): SqlSession 인터페이스 타입의 객체를 반환하는 메소드. boolean 타입을 인자로 전달한다
